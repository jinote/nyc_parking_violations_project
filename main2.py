...
parser.add_argument("--init_offset", type=int, help="Initiating offset")
...
if __name__ == '__main__':
    ...
    client = Socrata("data.cityofnewyork.us", APP_TOKEN, timeout=180)
    # Required because some offsets were taking more than 10 secs (default)
    # Creating schema/index in ES
    create_index()

    # Deducing num_pages and initial offset
    num_pages = add_num_pages()
    init_offset = add_init_offset()
    ...
    for page in range(0, num_pages):
        ...
        print("Final request being posted: " + str(len(es_rows)))
        try:
            start_time_post = time.time()
            helpers.bulk(es_client, es_rows)  # ES bulk call
            end_time_post = time.time()
            print("Time to make EC call for page : " + str(page) + " " + str(end_time_post - start_time_post))
        except Exception as e:
            print(f"Failed to upload to ES! {e}")
        end = time.time()
